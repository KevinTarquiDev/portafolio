...
| 4    | Hecha  | 2026-09-15 | `bun run verify` en verde. **Alcance ajustado deliberadamente** (ver nota abajo): en vez de escenas ancladas (pin) con crossfade, se implementaron reveals y transformaciones ligadas al scroll con el mismo repertorio de GSAP (timelines, ScrollTrigger, matchMedia, scrub, parallax, rotación), priorizando robustez/accesibilidad sobre la fidelidad literal del mockup animado. Verificado con Chrome headless: con `--force-prefers-reduced-motion` no se solicita ningún chunk de gsap/ScrollTrigger (0 coincidencias en el netlog); en modo normal se añade la clase `motion` a `<html>`, sin errores de consola; JS con motion ≈ 47 KB gzip (gsap 27 KB + ScrollTrigger 17 KB + wrapper 2.4 KB), bien bajo el objetivo de 70 KB; sin motion, solo el wrapper (~2.4 KB), bajo el objetivo de 5 KB. **Bug real encontrado y corregido durante la verificación**: animar `opacity` sobre el `<h1>` gigante del hero degradaba visualmente el texto (blanco → gris) de forma persistente incluso después de limpiar los estilos; se resolvió animando el nombre solo por posición (`y`), nunca por opacidad, y se añadió una red de seguridad (`ensureVisible`) que fuerza `opacity`/`transform` limpios tras un timeout en el resto de reveals, para que ninguna animación interrumpida deje contenido invisible                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | No se implementaron los pins de scroll horizontal (EcuStock) ni el crossfade anclado de Experiencia — ver el alcance ajustado arriba; si se quiere esa fidelidad exacta del mockup, es trabajo adicional a decidir. Medición de fps real en Chrome DevTools > Performance durante un scroll completo: no se pudo ejecutar desde este entorno (sin navegador interactivo); pendiente confirmar ~60 fps sin long tasks >50 ms                                                                                                                                                                                                                                                                                               |
| 5    | Hecha  | 2026-09-15 | `bun run verify` en verde (80 tests, 0 fail: contact.ts, brevo.ts, contact-handler.ts). **E2E real ejecutada contra `astro dev` con el `.env` real** (`scripts/e2e-contact.ts`): envío válido → 200 (Brevo aceptó el mensaje, 2xx); campos inválidos → 422 con `errors.name="required"` y `errors.email="invalid_email"`; honeypot relleno → 200 sin llamar a Brevo; `Origin` ajeno → 403; envío sin JS → 303 a `/es/contact/sent/`. Manejo de errores de Brevo probado con una segunda instancia de `astro dev` en el puerto 4325 con `BREVO_API_KEY=invalid-key-for-test` **solo en la variable de entorno del proceso** (`.env` sin tocar): respondió 502 `{ok:false,error:"send_failed"}`, sin exponer la clave inválida. `secrets:check` sin `--allow-missing`: 0 coincidencias del valor real de `BREVO_API_KEY` en `.vercel/output`, `dist`, `public` ni `.tmp/logs`. **Hallazgo documentado**: Astro 7 trae activado por defecto `security.checkOrigin` (rechaza con 403 cualquier POST de tipo formulario cuyo header `Origin` no coincida exactamente con el origen de la petición, incluso sin `Origin`); la validación de `Origin` propia en `contact-handler.ts` queda como defensa adicional, ya cubierta por 6 tests unitarios, pero Astro la aplica primero                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | **PENDIENTE DE VERIFICACIÓN MANUAL** (el agente no tiene acceso a la bandeja): confirmar en `CONTACT_RECIPIENT_EMAIL` que llegaron los correos de prueba, que el remitente es `CONTACT_SENDER_NAME <CONTACT_SENDER_EMAIL>` y que Responder dirige al email usado en cada prueba (`test-error@example.com` y los `e2e-*@example.com` generados)                                                                                                                                                                                                                                                                                                                                                                            |
| 6    | Hecha  | 2026-09-15 | `bun run verify` en verde (87 tests, 0 fail; incluye `buildCvDocument`). `bun run cv:generate` produce `public/cv/kevin-tarqui-cv-es.pdf` y `-en.pdf`, 2 páginas cada uno (`file` confirma "PDF document, 2 page(s)"), estilo Harvard con secciones Perfil/Experiencia/Proyectos/Educación/Habilidades/Idiomas, email/LinkedIn/GitHub como anotaciones de enlace independientes, sin ninguna extensión `x-*` (verificado por test). Revisión visual con el Read tool sobre el PDF real (ambos locales, ambas páginas): tipografía Times, acentos correctos (á, é, ó, í), fechas con `formatMonthRange`/`formatYearRange`, sin desbordes ni solapes. `build:check` extendido: confirma que el PDF de cada idioma existe, empieza por `%PDF-` y que el HTML de cada locale enlaza al PDF correspondiente. `public/cv/*.pdf` añadido a `.gitignore`; `bun run dev`/`build` regeneran el CV antes de arrancar/compilar. **Bug real encontrado durante la revisión general del diseño (reportado por el usuario) y corregido en esta fase**: en el hero, la mitad sólida y la mitad de contorno de "Tarqui" (y "Kevin Tarqui" en inglés) quedaban visualmente separadas por una línea de fondo visible en el punto de corte, porque una capa estaba en flujo normal y la otra absoluta con `height:1lh`, dando un desajuste de subpíxel; se unificaron ambas capas como `absolute inset-0` sobre un contenedor con `height:1lh`, confirmado sin costura visible en capturas a 1440px en ambos idiomas                                                                                                                                                                                                                                                                                                                                                                                                         | No hay extractor de texto (`pdftotext`) disponible en este entorno para confirmar automáticamente que el texto es seleccionable en orden lógico; la inspección visual de ambas páginas en ambos idiomas no mostró nada que lo sugiera, pero queda: "Abrir cada PDF, seleccionar todo el texto y confirmar que se copia en el orden correcto (nombre, contacto, perfil, experiencia…) y que los acentos se ven bien"                                                                                                                                                                                                                                                                                                       |
| 7    | Hecha  | 2026-09-15 | `bun run verify` en verde (93 tests, 0 fail; incluye `seo.ts`). `build:check` (Fase 7) verificado sobre el build real: canonical propio y hreflang recíprocos (es/en/x-default) en `/es/` y `/en/`; meta `og:*` y `twitter:*` con URLs absolutas (`https://kevintarqui.vercel.app/...`, confirmado con grep sobre el HTML generado); JSON-LD `@type: "Person"` parseable; `sitemap.xml` con ambas URLs y `xhtml:link` alternates; `robots.txt` con `Disallow: /api/` y `Sitemap:` absoluto. Páginas de resultado del contacto y 404 con `noindex` (vía el nuevo prop `Seo.noindex`, reemplazando el `<meta slot="head">` manual). Assets de marca generados a mano con Chrome headless (documentado en `scripts/generate-brand-assets.md`, sin datos profesionales, solo el monograma "K"/"KT" + punto verde): `favicon.svg` (vectorial), `favicon.ico` y `apple-touch-icon.png` regenerados, `og-image.png` 1200×630 (195 KB, bajo el límite de 200 KB) — revisados visualmente con el Read tool                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | **PENDIENTE DE VERIFICACIÓN MANUAL** (validadores externos, no ejecutables desde este entorno sin el sitio ya desplegado): "Pegar la URL de `/es/` ya desplegada en https://validator.schema.org y confirmar Person sin errores"; "Tras el deploy, revisar la vista previa en https://www.opengraph.xyz para `/es/` y `/en/`"                                                                                                                                                                                                                                                                                                                                                                                             |
| 8    | Hecha  | 2026-09-15 | `bun run verify` en verde (93 tests, 0 fail; `check-build.ts` extendido con `checkPhase8` que valida `vercel.json`). **Bug real encontrado y corregido** (auditoría Lighthouse, no reportado por el usuario): accesibilidad 96/100 en `/es/` móvil por 3 patrones `color-contrast` — año no activo en la lista mobile de Experiencia (`text-outline`, ratio 2.93 sobre 3:1 requerido), el `<div>` de datos del ítem no activo con `opacity-60` que degradaba por debajo del umbral incluso el `text-muted` ya conforme, y las keywords no activas de Stack (`text-outline`). Corregido sustituyendo el contorno/opacidad por `text-muted` sólido (ya validado en `contrast.test.ts`) en los tres casos, preservando la des-énfasis visual del elemento no activo. Tras el fix, con procesos `bunx serve`/`astro dev` residuales de fases previas detenidos (contaminaban la medición de rendimiento — hasta 11 instancias abiertas): Lighthouse móvil `/es/` → Performance 99, Accessibility 100, Best Practices 100, SEO 100; `/en/` móvil → Performance 99, Accessibility 100, Best Practices 100, SEO 100; Lighthouse desktop `/es/` → Performance 100, Accessibility 100, Best Practices 100, SEO 100 (supera los objetivos ≥95 móvil / ≥98 desktop). CLS = 0 en ambos runs. Comprobado con Chrome headless vía CDP (sin depender de la extensión, no conectada en este entorno): sin scroll horizontal en 320/360/390/414/768/1024/1280/1440/1920/2560 px en `/es/` y `/en/`; con `prefers-reduced-motion: reduce` no se solicita ningún recurso de GSAP; skip link presente y funcional (`href="#main"`); regla `:focus-visible` presente en el CSS compilado. `vercel.json` (cache immutable de `/_astro/*` ya la escribe el propio adaptador en `config.json`; las cabeceras de seguridad las aplica la plataforma Vercel en el despliegue real, no el adaptador — documentado en `checkPhase8`) | **PENDIENTE DE VERIFICACIÓN MANUAL** (no ejecutable desde este entorno): "Con NVDA (Windows) o VoiceOver, recorrer `/es/` por encabezados (H) y comprobar el orden de lectura en Experiencia, Proyecto destacado y Stack, y que no se lee texto decorativo duplicado"; "Chrome DevTools > Performance, grabar scroll completo en desktop y mobile reales y confirmar ~60 fps sin long tasks >50 ms"; axe-core no se pudo ejecutar (sin `@axe-core/cli` disponible ni extensión Chrome conectada) — Lighthouse cubrió `color-contrast` y demás reglas de accesibilidad automatizables con 100/100, pero un pase de Axe DevTools manual en `/es/` y `/en/` queda como confirmación adicional recomendada                    |
| 9    | Hecha  | 2026-09-15 | `bun run verify` en verde (formato, `resume:validate` 2/2, `bun test`, `astro check` 0 errores, build, `build:check`). `bun run secrets:check` **sin** `--allow-missing`, ejecutado contra el `.env` real: 0 coincidencias del valor de `BREVO_API_KEY` en `.vercel/output`, `dist`, `public` ni `.tmp/logs`. `.env.example` solo con nombres, sin valores. `git ls-files` confirma que no hay `.env` ni PDFs generados trackeados (`.gitignore` los excluye: `.env`, `.env.*`, `public/cv/*.pdf`). `README.md` reescrito por completo (sustituye el starter de Astro): stack, requisitos, scripts, variables de entorno, fuente de datos, CV, assets de marca, pasos de despliegue en Vercel y verificación. Se probó `bunx vercel whoami`: CLI descargable pero sin sesión autenticada ni proyecto enlazado en este entorno, así que el deploy real no se ejecutó                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | **PENDIENTE DE VERIFICACIÓN MANUAL** (requiere una cuenta de Vercel autenticada, no disponible en este entorno): "En Vercel: crear el proyecto con Framework Astro, Install Command `bun install`, Build Command `bun run build`, Node 22.x; configurar `BREVO_API_KEY`, `CONTACT_SENDER_EMAIL`, `CONTACT_SENDER_NAME`, `CONTACT_RECIPIENT_EMAIL` y `SITE_URL` en Production y Preview"; "En la URL de Preview/Production: confirmar que `/` redirige a `/es/`, que el formulario de contacto envía y llega el correo, que los botones .Descargar CV. bajan el PDF del idioma correcto, que `/sitemap.xml` y `/robots.txt` responden, y que la vista previa de Open Graph se ve correctamente en LinkedIn/WhatsApp/Slack" |
